...
class DocumentHierarchyBuilder:
        ...
        if self._features is None:
            ...
            for heading in self.headings:
                feature_dict = {
                    ...
                    "bold": 1.0 if heading['is_bold'] else 0.0,
                    "italic": 1.0 if heading['is_italic'] else 0.0,
                    "level_latin" : infer_header_level_roman(heading["text"]),
                    "level_alpha" : infer_header_level_letter(heading["text"]),
                    "level_numerical" : infer_header_level_numerical(heading["text"]),
    ...
    def infer_by_numbering(self, start_numbering_at_root=True, ignore_numbering_of_title=False) -> HierarchicalHeader:
        numbering_types = ["level_latin", "level_alpha", "level_numerical"]
        ...
        if ignore_numbering_of_title:
            ...
            font_size_clusters = self.cluster_headings_dbscan()
            # assume biggest font size is title:
            title_indices = [i for i, l in font_size_clusters.items() if l == 1]
        ...
        for f in features:
            if len([k for k in numbering_types if f[k]]) > 1:
                raise Exception("Inconsistent numbering - either reading order is messed up or header numbering is not intended. Extract hierarchy from style only.")
                        ...
                        if self._gt_hierarchical(current_level, new_level):
                            if len(new_level) > len(current_level):
                                if current_level == new_level[:len(current_level)]:
                                    ...
                                    new_parent = current
                                    while new_parent.parent is not None and hasattr(new_parent, k) and (getattr(new_parent, k) != new_level[:len(getattr(new_parent, k))]):
                                        new_parent = new_parent.parent
                                    break
                            else:
                                new_parent = current
                                while new_parent.parent is not None and (len(getattr(new_parent, k)) >= len(new_level) or (hasattr(new_parent, k) and (getattr(new_parent, k) != new_level[:len(getattr(new_parent, k))]))):
                                    new_parent = new_parent.parent
                                break
                        else:
                            raise Exception("Inconsistent numbering - either reading order is messed up or header numbering is not intended. Extract hierarchy from style only.")
                        ...
                        while True:
                            ...
                            if last_level and self._gt_hierarchical(last_level, new_level):
                                ...
                                if len(new_level) > len(last_level):
                                    ...
                                    if new_level[:max(len(last_level)-1, 0)] != last_level[:max(len(last_level)-1, 0)]:
                                        raise Exception("Inconsistent numbering - something might have gone wrong during parsing.")
    ...
    def infer_by_clustering(self) -> HierarchicalHeader:
        heading_to_level = self.cluster_headings_dbscan()
        root = HierarchicalHeader()
        current = root
        ...
        def gt_stylistic(level_fontsize: int, style_attrs: list[str], ref: HierarchicalHeader):
            ...
            if level_fontsize < ref.level_fontsize:
                return False
            if len(style_attrs) > len(ref.style_attrs):
                return True
            return False
        ...
        def eq_stylistic(level_fontsize: int, style_attrs: list[str], ref: HierarchicalHeader):
            if ref.level_fontsize is None:
                return False
            if level_fontsize == ref.level_fontsize and len(style_attrs)  == len(ref.style_attrs):
                return True
            return False
        ...
        for i, this_style in enumerate(self.style_features):
            ...
            this_style_attr = [k for k in ["bold", "italic"] if this_style[k]]
            if gt_stylistic(this_fs_level, this_style_attr, current):
                ...
                if current.parent is not None:
                    ...
                    raise Exception("Hierarchy demands equal level heading, but no common parent was found!")
            else:
                # go back up in hierarchy and try to find a new parent
                new_parent = current
                while new_parent.parent is not None and (not gt_stylistic(this_fs_level, this_style_attr, new_parent) or eq_stylistic(this_fs_level, this_style_attr, new_parent)):
                    new_parent = new_parent.parent
                # print(f"fit parent for : {this_fs_level, this_style_attr} parent: {new_parent.level_fontsize, new_parent.style_attrs}")
            new_obj = HierarchicalHeader(index=i, text=self.headings[i]["text"], parent=new_parent, level_fontsize=this_fs_level, style_attrs=this_style_attr)
            new_parent.children.append(new_obj)
            current = new_obj

        return root

    def cluster_headings_dbscan(self) -> Dict[int, int]:
        ...
        for eps in eps_grid:
            for min_samples in min_samples_grid:
                ...
                if len(set(labels)) > 1 and len(set(labels)) != len(labels):  # At least two clusters required for silhouette
...
def create_toc(headings):
    ...
    try:
        if (root_node := builder.infer_by_numbering()) is not None:
            return root_node
    except Exception:
        pass
    print("falling back to cluster-based")
    root_node = builder.infer_by_clustering()
    return root_node
